...
float Engine::Text::GetTextWidth() const
{
	float width = 0.0f;
	for (const char& c : text)
	{
		if (TextCharacters.find(c) != TextCharacters.end()) {
			TextCharacter ch = TextCharacters.at(c);
			width += (ch.Advance >> 6) * scale; // Advance is in 1/64 pixels, so we bitshift by 6 to convert
		}
	}
	return width;
}
float Engine::Text::GetTextHeight() const
{
	...
	for (const char& c : text)
	{
		if (TextCharacters.find(c) != TextCharacters.end()) {
			TextCharacter ch = TextCharacters.at(c);
			// Calculate the height of the character in pixels
			float charHeight = (ch.Bearing.y - (ch.Size.y - ch.Bearing.y)) * scale;
